...
- [ ] **Proxy-Server Implementierung**: Ersetzung des Platzhalters in `apps/proxy-server/src/index.ts` durch einen echten Fastify-Server (Subagent #7).
- [ ] **API-Endpunkte**: Implementierung von `/transcribe` (Chirp 3) und `/enrich` (Gemini 2.5 Flash).
...
- [ ] **Intent Classification**: Echte Logik zur Unterscheidung zwischen INVOICE und ANALYTICS Intents implementieren (Subagent #6).
...
- [ ] **Export-Funktion**: Echte Implementierung des CSV/JSON Exports auf dem Dashboard.
...
---
*Status: Stand 24. Januar 2026 - Basierend auf Code-Audit und Subagent-Platzhaltern.*
