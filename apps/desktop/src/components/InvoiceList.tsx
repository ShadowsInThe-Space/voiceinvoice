...
interface InvoiceRowProps {
  ...
  isSelected: boolean;
  onSelect: (invoice: Invoice) => void;
  onKeyDown: (e: React.KeyboardEvent, invoice: Invoice) => void;
 ...
 * Memoized to prevent unnecessary re-renders of the entire list when
 * parent state changes (e.g. selection or filter text input).
 */
const InvoiceRow = React.memo(({
  ...
  return (
    <tr
      ...
      onKeyDown={(e) => onKeyDown(e, invoice)}
      tabIndex={0}
      className={cn(
    ...
    >
      ...
      {showDeleteButton && (
        <td className="px-8 py-6 text-right">
          <button
            onClick={(e) => onDeleteClick(e, invoice.id)}
