...
export class VoiceInvoicePipeline {
  ...
  private async createInvoiceFromParsed(
    ...
    // DSGVO: Anonymize transcription before storing in database
    // This removes PII (emails, phone numbers, IBANs, tax IDs) from the stored text
    const anonymizationResult = anonymize(transcription, { strategy: 'redact' });

    const invoiceInput: CreateInvoiceInput = {
      customerId,
      items,
      taxRate: this.defaultTaxRate,
      transcription: anonymizationResult.anonymizedText,
    };
