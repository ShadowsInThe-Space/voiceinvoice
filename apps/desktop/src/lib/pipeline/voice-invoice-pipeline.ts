...
export interface PipelineResult {
  ...
  /** The parsed invoice data (if review is required) */
  parsedInvoice?: ParsedInvoice;

  /** Whether the result requires user review (confidence < 0.85) */
...
export class VoiceInvoicePipeline {
  ...
  private async processTranscriptionInternal(transcription: string): Promise<PipelineResult> {
    ...
    // If confidence is low, return parsed data for user review instead of saving
    if (parseResult.confidence < 0.85) {
      return {
        success: true,
        parsedInvoice: parseResult.invoice,
        requiresReview: true,
        transcription,
