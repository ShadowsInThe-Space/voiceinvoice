...
const STORAGE_KEYS = {
  ...
  n8nWebhookUrl: 'voiceinvoice_n8n_webhook_url',
};
...
export default function SettingsPage(): React.ReactElement {
  ...
  useEffect(() => {
    ...
    const savedWebhook = localStorage.getItem(STORAGE_KEYS.n8nWebhookUrl);
  ...
  const handleSave = useCallback(
    async (e: FormEvent) => {
      ...
      try {
        ...
        if (n8nWebhookUrl) {
          localStorage.setItem(STORAGE_KEYS.n8nWebhookUrl, n8nWebhookUrl);
        } else {
          localStorage.removeItem(STORAGE_KEYS.n8nWebhookUrl);
        }
        ...
        <section role="group" aria-labelledby="integration-section">
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow p-6 space-y-4">
            <div>
              ...
              />
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                URL fuer den n8n Webhook, an den Daten gesendet werden sollen
              </p>
            </div>
          </div>
        </section>
